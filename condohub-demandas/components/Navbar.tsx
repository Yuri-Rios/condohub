...
export default function Navbar() {
  ...
  const itensDocumentos: ItemMenu[] = [
  ...
  const itensOperacao: ItemMenu[] = podeAdministrar ? [
  ...
  function secaoCondominio() {
    const grupos = [{ titulo: "Documentos", itens: itensDocumentos }, { titulo: "Gestão", itens: itensGestao }, { titulo: "Operação", itens: itensOperacao }].filter(grupo=>grupo.itens.length);
    if (!grupos.length) return null;
    return <div><p className="px-2 text-xs font-bold uppercase tracking-wider text-slate-400">Condomínio</p><div className="mt-3 grid gap-4 border-l border-slate-200 pl-3">{grupos.map(grupo=><div key={grupo.titulo}><p className="px-2 text-[10px] font-bold uppercase tracking-widest text-slate-400">{grupo.titulo}</p><div className="mt-1 grid gap-1">{grupo.itens.map(item=><Link key={item.href} href={item.href} onClick={()=>setMenuAberto(false)} className={`flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold ${classeLink(item.href)}`}>{item.label}{item.badge?<span className="rounded-full bg-rose-500 px-2 py-0.5 text-xs text-white">{item.badge}</span>:null}</Link>)}</div></div>)}</div></div>;
  }
  ...
  return (
    ...
    <aside id="menu-desktop" className="fixed inset-y-0 left-0 z-40 hidden w-72 flex-col border-r border-slate-200 bg-white shadow-[8px_0_30px_rgba(15,23,42,.05)] lg:flex">
      ...
      <div className="flex-1 overflow-y-auto p-4"><div className="grid gap-6">{grupoMenu("Principal",itensPrincipais)}{secaoCondominio()}{grupoMenu("Configurações",itensConfiguracoes)}{grupoMenu("Plataforma",itensPlataforma)}</div></div>
