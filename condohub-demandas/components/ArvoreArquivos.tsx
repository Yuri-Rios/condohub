...
export default function ArvoreArquivos<T extends ArquivoOrganizado>({ arquivos, renderArquivo, pastas=[], preservarOrdem=false }: { arquivos:T[]; renderArquivo:(arquivo:T)=>ReactNode; pastas?:string[]; preservarOrdem?:boolean }) {
  const raiz = montarArvore(arquivos,pastas) as Pasta;
  // A API já entrega balancetes por competência decrescente.
  const ordenar = (itens: ArquivoOrganizado[]) => preservarOrdem ? itens : [...itens].sort((a,b)=>a.nome_arquivo.localeCompare(b.nome_arquivo,"pt-BR"));
  ...
  function pasta(no:Pasta,nivel:number):ReactNode { return <details key={no.caminho} open className={nivel?"ml-3 border-l border-slate-200 pl-3":""}><summary className="flex cursor-pointer list-none items-center gap-2 rounded-lg px-3 py-2.5 font-bold text-slate-800 hover:bg-slate-50"><span className="text-amber-500">▰</span><span className="truncate">{no.nome}</span><span className="ml-auto text-xs font-normal text-slate-400">{no.arquivos.length+Array.from(no.pastas.values()).reduce((s,p)=>s+p.arquivos.length,0)}</span></summary>{no.arquivos.length>0&&<div className="overflow-hidden rounded-xl border border-slate-200 bg-white">{ordenar(no.arquivos).map(linha)}</div>}<div className="mt-2 grid gap-2">{Array.from(no.pastas.values()).sort((a,b)=>a.nome.localeCompare(b.nome,"pt-BR")).map(p=>pasta(p,nivel+1))}</div></details> }
  return <div className="grid gap-3">{raiz.arquivos.length>0&&<div className="overflow-hidden rounded-xl border border-slate-200 bg-white">{ordenar(raiz.arquivos).map(linha)}</div>}{Array.from(raiz.pastas.values()).sort((a,b)=>a.nome.localeCompare(b.nome,"pt-BR")).map(p=>pasta(p,0))}</div>;
}
